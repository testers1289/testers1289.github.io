...
function updateAuthUI(){
  ...
  if (state.me){
    ...
    const btn = document.createElement('button'); btn.textContent='Logout'; btn.style.marginLeft='8px'; btn.addEventListener('click', async () => { await fetch(API_BASE + '/api/logout', { method:'POST', credentials:'include' }); state.me=null; updateAuthUI(); });
...
async function selectChannel(id) {
  ...
  msgs.forEach(m => {
    ...
    b.addEventListener('click', async () => {
      await fetch(API_BASE + `/api/channels/${id}/messages/${m.id}`, { method: 'DELETE', credentials: 'include' });
      selectChannel(id);
    });
    li.appendChild(b);
    ul.appendChild(li);
